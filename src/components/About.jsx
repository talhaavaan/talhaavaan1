...
export default function About() {
  ...
  // Skills bloom (0.85 → 1)
  const headlineY      = useTransform(progress, [0, 0.25], ['40vh', '0vh']);
  const headlineScale  = useTransform(progress, [0, 0.25, 0.4], [1.15, 1, 0.82]);
  const headlineOpacity= useTransform(progress, [0, 0.12, 0.34, 0.42], [0, 1, 1, 0]);
  const headlineBlur   = useTransform(progress, [0.34, 0.42], ['0px', '8px']);

  const portraitScale  = useTransform(progress, [0, 0.25], [0.65, 1]);
  const portraitRotate = useTransform(progress, [0, 1], [-8, 8]);
  const portraitY      = useTransform(progress, [0.25, 0.85], ['0%', '-12%']);
  const portraitOpacity= useTransform(progress, [0.38, 0.46, 0.82, 0.88], [1, 0, 0, 0]);

  const railX          = useTransform(progress, [0.45, 0.85], ['10%', '-78%']);
  const railOpacity    = useTransform(progress, [0.42, 0.5, 0.82, 0.9], [0, 1, 1, 0]);

  const skillsOpacity  = useTransform(progress, [0.88, 0.96], [0, 1]);
  const skillsY        = useTransform(progress, [0.88, 1], [60, 0]);
...
>
  ...
  <span style={{
    position: 'absolute', bottom: '6rem', left: '1.2rem',
    ...
    fontSize: '.7rem', letterSpacing: '.3em', textTransform: 'uppercase',
        ...
        <motion.p
          ...
          opacity: useTransform(progress, [0.14, 0.24, 0.34, 0.42], [0, 1, 1, 0]),
          y: useTransform(progress, [0.14, 0.24], [40, 0]),
          }}
        >
